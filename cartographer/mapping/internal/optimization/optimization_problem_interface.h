...
public:
  ...
  OptimizationProblemInterface &
  operator=(const OptimizationProblemInterface &) = delete;
  ...
  virtual void
  Solve(const std::vector<Constraint> &constraints,
  ...
  virtual const std::map<std::string, transform::Rigid3d> &
  landmark_data() const = 0;
  virtual const sensor::MapByTime<sensor::ImuData> &imu_data() const = 0;
  virtual const sensor::MapByTime<sensor::OdometryData> &
