...
public:
  ...
  FastCorrelativeScanMatcher3D &
  operator=(const FastCorrelativeScanMatcher3D &) = delete;
  ...
  MatchFullSubmap(const Eigen::Quaterniond &global_node_rotation,
...
private:
  ...
  GenerateLowestResolutionCandidates(const SearchParameters &search_parameters,
  ...
  transform::Rigid3f
  GetPoseFromCandidate(const std::vector<DiscreteScan3D> &discrete_scans,
